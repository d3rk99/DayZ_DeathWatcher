...
class GuiApplication:
    ...
    def _ensure_initial_paths(self) -> None:
        ...
        if self._needs_full_setup and self._path_dialog is None and self._bot_dialog is None:
            self._set_ready(False)
            self._show_server_root_dialog()
            return
    ...
    def _show_server_root_dialog(self) -> None:
        ...
        def _on_complete() -> None:
            from services.bot_fields import REQUIRED_BOT_KEYS
            ...
            missing = [
                ...
                if not str(self.config_manager.data.get(key, "")).strip()
            ]
            if missing:
                self._show_bot_setup_dialog()
            else:
        ...
        self._path_dialog = ServerRootSetupDialog(
            self.root,
            self.config_manager,
            server_count=5,
