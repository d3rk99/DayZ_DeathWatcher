...
router.get('/discord/callback', async (req, res) => {
  ...
  try {
    ...
    const now = nowIso();
    if (existing) {
      db.prepare('UPDATE users SET discord_username = ?, display_name = ?, last_login_at = ? WHERE id = ?')
        .run(discordUser.username, discordUser.global_name || discordUser.username, now, existing.id);
      (req as any).session.userId = existing.id;
    } else {
      const role = APP_CONFIG.discord.adminIds.includes(discordUser.id) ? 'admin' : 'player';
      const result = db
        .prepare('INSERT INTO users (discord_id, discord_username, display_name, role, created_at, last_login_at) VALUES (?, ?, ?, ?, ?, ?)')
        .run(discordUser.id, discordUser.username, discordUser.global_name || discordUser.username, role, now, now);
